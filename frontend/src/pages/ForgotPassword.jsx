      ...
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-100 premium-shadow p-6 sm:p-8 space-y-8">
        ...
        <form onSubmit={codeSent ? handleReset : handleRequest} className="space-y-4">
          <div className="relative">
            ...
            <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address" className="w-full bg-slate-50 rounded-2xl pl-12 pr-4 py-4 outline-none" />
          </div>

          {codeSent && (
            <>
              <input required value={token} onChange={(e) => setToken(e.target.value)} placeholder="Reset code" className="w-full bg-slate-50 rounded-2xl px-4 py-4 outline-none" />
              <input required type="password" minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="New password" className="w-full bg-slate-50 rounded-2xl px-4 py-4 outline-none" />
            </>
          )}
